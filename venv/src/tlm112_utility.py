...
def connectAurora(query,FOLDER_PATH):
    ...
    for (driver_id, trip_id, s3key) in cursor:
        ...
        if len(df_result2temp) == 0:
            df_result2 = df_result2.append({'driver_id': driver_id, 'trip_id': trip_id, 's3_key': s3key, 'FOLDER_PATH':FOLDER_PATH},
                                           ignore_index=True)
        else:
...
def processCSVtoGetS3key(FOLDER_PATH):
    ...
    import mysql.connector
    exampleList = pd.read_csv(FOLDER_PATH + "pmanalysis_tlm_112/non-geotab/data1000.csv",
                              index_col=False, nrows=3)
...
def processDriver(driver_id, session_id, trip_id):
...
def copyFilesfromS3toRegressionServer(driver_id, trip_id, s3listbyTripId, FOLDER_PATH):
    ...
    try:
        ...
        log = processDriver(driver_id, session_id, trip_id)
