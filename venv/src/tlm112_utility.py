...
def processCSVtoGetS3key(FOLDER_PATH):
    ...
    for (driver_id, trip_id, s3key) in cursor:
        ...
        result = "select driver_id, trip_id,s3_key from trip_file where "
        query = []
    df_result.to_csv(FOLDER_PATH+"PMANALYSIS_TLM_112/weekly_trips_final.csv",
                     index=False)
    threadjobs = []
    ...
    exampleList["STATUS"] = ""
    for item in result:
        exampleList.loc[(exampleList['trip_id'] == item[1]) & (exampleList['driver_id'] == item[0]),['PM_COUNT']] = item[3]
        exampleList.loc[(exampleList['trip_id'] == item[1]) & (exampleList['driver_id'] == item[0]), ['STATUS']] = item[2]
